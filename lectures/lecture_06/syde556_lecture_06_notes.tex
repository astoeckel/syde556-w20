...
\Note{We have now discussed the first two principles of the Neural Engineering Framework: Representation and Transformation. These two principles allow us to build feed-forward networks, i.e.,~networks that can -- mathematically speaking -- be described as acyclic directed graphs.}
...
The above equations illustrate how recurrent connections are implemented and what the represented value of the recurrently connection is. To get some intuition for the dynamics of such a system, we can try test different feedback functions $f(\vec x)$. \Cref{fig:dynamics_example} depicts the behaviour of a population of LIF neurons representing a one-dimensional value along with various feedback functions and various synaptic filter time constants.
...
\Note{\emph{Assumptions.} As so often, we will ignore the fact that we are dealing with vectorial quantities in the following subsection and instead assume that $d = 1$. That being said, all equations are also holding for higher-dimensional representations.
...
\end{align*}
\Note{\emph{Canonical dynamical system and phase portrait.} The canonical form of time-independent a dynamical system
\begin{align*}
	\frac{\mathrm{d}}{\mathrm{d}t} x(t) &= \phi(u(t), x(t)) \,
\end{align*}
where $\phi(u, x)$ is an arbitrary function mapping an input $u$ and the state $x$ onto a state differential. We can draw the function $\phi(u, x)$ as a so called \emph{phase portraits} by drawing an arrow corresponding to the state update in an $x$-$u$ coordinate system.}
...
\begin{align*}
	\mat A &= 0 && \mat B &= \mat I \\
	\Leftrightarrow \mat A' &= \mat I  & \mat B &= \tau \mat I
\end{align*}
...
\begin{align*}
	...
	\Leftrightarrow \mat A' &= \begin{pmatrix}1 & -\tau\omega \\ \tau \omega & 1\end{pmatrix}  && \mat B &= 0
\end{align*}
...
\begin{align*}
	...
	f'(\vec x) &= \mat I & g(\vec u) &= \tau \alpha \vec u
\end{align*}
...
\begin{align*}
	...
	\Leftrightarrow \mat A' &= \begin{pmatrix}1 & -\alpha\tau\omega \\ \alpha\tau \omega & 1\end{pmatrix}  && \mat B &= 0
\end{align*}
