...
\end{align*}
where $\lambda$ is the so called \enquote{regularisation factor}. Hence, in the case above, we are using $L_2$ regularisation with $\lambda = N \sigma^2$.
...
\hrulefill {\symbolfont 🚧} To be filled in {\symbolfont 🚧} \hrulefill
...
\begin{itemize}
	\item[\textellipsis] the encoders $\vec e_i$ should be uniformly sampled from the unit-sphere. This can be achieved by sampling each encoder component from a normal distribution (i.e.,~a Gaussian distribution with mean zero and variance one) and normalising the resulting vectors to length one.
...
\subsection{Example: Horizontal Eye Control (1D)}
