...
\begin{frame}{Problems With Current Approaches: Large-scale Neural Models}
	\begin{columns}[c]
		...
		\begin{itemize}
			...
			\end{itemize}
		\end{itemize}%
		\only<1-6>{\setlength\fboxsep{0.5em}\colorbox{white}{\begin{minipage}{\textwidth}\bfseries\color{white}{\symbolfont ⚠~}%
...
\begin{frame}{Problems With Current Approaches: Behavioural Models}
	\begin{columns}[c]
		...
		\end{itemize}%
		\vspace{0.25cm}
		\only<1-6>{\setlength\fboxsep{0.5em}\colorbox{white}{\begin{minipage}{\textwidth}\color{white}%
			\begin{itemize}
				\item[\color{white}\symbolfont ⚠] \color{white} \textbf{Maybe these shortcomings are okay.}
				\item[] \color{white} Do we understand the brain enough to derive bridging laws and to constrain theories?
...
\begin{frame}{The Controversy}
	\begin{columns}
		...
		\only<4->{\tiny\raggedleft\color{aluminium4} Source: \href{https://spectrum.ieee.org/tech-talk/semiconductors/devices/blue-brain-project-leader-angry-about-cat-brain}{IEEE Spectrum}, \enquote{Cat Fight Brews Over Cat Brain} (2019)\\}
		\end{overlayarea}
	\end{columns}
\end{frame}
