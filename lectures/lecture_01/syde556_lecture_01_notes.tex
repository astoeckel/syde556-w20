...
Since our models will get complex quite quickly, we must most certainly use some kind of computer simulation. Correspondingly, Theoretical and Computational Neuroscience often quite closely related.}
...
\Note{The term \emph{functional model} may be a little unfortunate since the term \emph{functional} is quite overloaded. As described above, we use \emph{functional} solely in the sense of \enquote{producing a behaviour} or \enquote{solving a cognitive task}.}
...
Let's start with some basic facts about the human brain:\footnote{For those of you who still find this a little too abstract, take a look at this \YouTube[video showing an unfixed human brain]{jHxyP-nUhUY} (warning: dead human body parts). In case you'd like to experience this in real life, take PSYCH~784.}
\begin{itemize}
...
Advances in scanning techniques allow researchers to reconstruct neural networks down to a nanometer scale, which means that we will have to the \enquote{wiring diagram} of brain regions (see this \YouTube[National Geographics Video]{nvXuq9jRWKE} and this \YouTube[TEDx Talk]{F37kuXObIBU} by Jeff Lichtman).
...
Markham's rage did not -- as one might expect -- stem from the fact that IBM's \enquote{cat brain} did not do what one would expect from a functional brain (controlling behaviour), but merely from the fact IBM's neurons were \enquote{too simple} and therefore cannot possibly constitute a full brain model. Markham's HBP and its predecessor, the BlueBrain project, make use of physiologically and anatomically extremely detailed neuron models that are (in part) randomly connected. As of 2020, the HBP has similarly failed to produce any \enquote{functional} brain model.
...
\begin{multicols}{2}
	\begin{itemize}
		...
		\item \YouTube[Spaun: addition by counting]{WNnMhF7rnYo}
