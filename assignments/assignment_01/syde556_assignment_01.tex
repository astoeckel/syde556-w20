...
\begin{document}
	...
	\begin{center}
		{\large\bf SYDE 556/750 --- Assignment 1}\\
	...
	\colorbox{GhostWhite}{\begin{minipage}{\textwidth}{\vspace{0.25cm}\centering\large\textsc{Notes}\\}\begin{minipage}{0.95\textwidth}
		\vspace{0.25cm}
		\begin{itemize}
			\item This assignment is worth 20 marks (20\% of the final grade). The number of marks for each question is indicated in brackets to the left of each question.
			\item There is a late penalty of one mark per day late.
			\item Please use Python 3 along with the \texttt{numpy} and \texttt{matplotlib} libraries to solve these assignments. You are highly encouraged to fill out this Jupyter Notebook template:
			\begin{center}
				\url{https://student.cs.uwaterloo.ca/~astoecke/syde556/assignment_01_template.ipynb}
			\end{center}
			\item Please hand in \textit{both} a PDF version of your results, as well as your executable code/the \texttt{ipynb} file. When using Jupyter Notebook, make sure your code runs without errors by \textit{restarting the kernel and re-running all cells} before submitting!
			\item Place your files in an archive named \texttt{<Lastname>.Assignment1.zip}, containing files \texttt{<Lastname>.Assignment1.Code.ipynb} and \texttt{<Lastname>.Assignment1.pdf}, where \texttt{<Lastname>} should be replaced with your last name.
			\item Send your assignments to \texttt{astoecke@uwaterloo.ca} using your University of Waterloo email account. The deadline is at \texttt{23:59 EST} on the date indicated above.
	...
	\begin{enumerate}[a)]
		...
		\begin{itemize}
			\item[{\symbolfont 🖈}] As in the previous question, $\sigma = 0.2 \max(A)$.
		\end{itemize}
	...
	\begin{enumerate}[a)]
		...
		\item \Marks{1.5} \textit{Impact of noise.} Generate the same four plots as in 1.1f) (adding/not adding noise to $A$, accounting/not accounting for noise when computing $\vec d$), and report the RMSE both with and without noise.
	\end{enumerate}
	...
	\begin{enumerate}[a)]
		...
		\item \Marks{0.5} \textit{Computing the identity decoder.} Use LIF neurons with the same properties as in question 1.3. When computing the decoders, take into account noise with $\sigma = 0.2\max{A}$. Plot the decoders in the same way you plotted the encoders.
		\begin{itemize}
